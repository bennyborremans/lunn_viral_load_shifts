...
# clunes spillovers only
spillover = spillover %>% 
  filter(all_spillovers_ID %in% c("all_058","all_060"))
...
if(run.again) {      
  ...
  for(j in 1:length(ct_thresholds)) {
    ...
    for(i in 1:permutations){
      ...
      hen_with_ci_by_month_perm$spillover_count[which(hen_with_ci_by_month_perm$sampling_cluster_median_date==as.Date("2017-08-08"))] = 1
    ...
    hen_with_ci_by_month$spillover_count[which(hen_with_ci_by_month$sampling_cluster_median_date==as.Date("2017-08-08"))] = 1
...
obs.out.f4 <- obs.out %>% 
  ...
  mutate(spillover = case_when(year == 2017 & month == 7 ~ "spillover", ## x1 - all_058
                               year == 2017 & month == 8 ~ "spillover", ## x1 - all_060
                               TRUE~ "no_spillover")) %>% 
...
plot.out
# ggsave(plot = plot.out,filename = "Figure S13 Clunes 20250715.png",width = 30, height = 24, dpi=300)
...
plot.out
# ggsave(plot = plot.out,filename = "Figure 4 Clunes only 20250715.png",width = 16, height = 8, dpi=300)
...
plot1 = ggplot() +
  ...
  scale_x_continuous(breaks = 28:40, limits = c(28,40)) +
...
plot3 = ggplot() +
  ...
  scale_x_continuous(breaks = 28:40, limits = c(28,40)) +
  scale_y_continuous(limits = c(3.9,25)) +
  xlab("Ct Threshold") +
  ylab("AIC") +
  theme_bw() +
  background_grid("none") +
...
plot.out
# ggsave(plot = plot.out,filename = "Figure S14 Clunes 20250715.png",width = 15, height = 6, dpi=600)
